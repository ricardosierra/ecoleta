...
echo "Enviando fatura de teste com layout oficial da Ecoleva...\n";
try {
    $customerId = asaasCreateCustomer('Teste Layout Ecoleva', 'sierra.csi@gmail.com', '12345678909', null);
    $payment = asaasCreatePayment($customerId, 5.00, date('Y-m-d', strtotime('+5 days')));
    $qrCode = asaasGetPixQrCode($payment['id']);
    
    $client = ['name' => 'Teste Layout Ecoleva', 'monthly_value' => 5.00, 'email' => 'sierra.csi@gmail.com'];
...
<div style="background:#F4F6F8;padding:40px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#FFFFFF;border-radius:4px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.05);">
    ...
    <div style="padding:40px 32px;">
      ...
      <p style="font-size:15px;color:#333333;line-height:1.6;margin-bottom:32px;">
...
HTML;
    ...
    $textBody = "Olá, {$client['name']}. Sua fatura da Ecoleva (R$ {$valorFormatado}, vencimento: {$vencimentoFormatado}) está disponível. Pix Copia e Cola: {$qrCode['payload']}";
    
    $mailOk = osSendMail($client['email'], 'Sua Fatura Mensal - Ecoleva', $htmlBody, $textBody);
    echo $mailOk ? " -> Teste enviado com sucesso!\n" : " -> Falha ao enviar o e-mail.\n";
} catch (Throwable $e) {
